...
async function loadOCMessages(ocFile) {
  ...
  async function processMessages(page, lastSentMessage, lastSentTimestamp) {
    try {
      const ocMessages = await loadOCMessages('lvye.txt');
      ...
      while (index < ocMessages.length) {
        ...
        const isLastSentMessageOld = currentTime - lastSentTimestamp > 6 * 10 * 1000;
    ...
    setTimeout(() => processMessages(page, lastSentMessage, lastSentTimestamp), 10000);
  }
